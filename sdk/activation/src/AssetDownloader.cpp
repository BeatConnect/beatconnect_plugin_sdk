...
// ==============================================================================
// PackageDownloader Implementation
// ==============================================================================

class PackageDownloader::Impl {
public:
    void configure(const DownloaderConfig& config) {
        assetDownloader.configure(config);
        this->config = config;
    }

    std::optional<PackageInfo> getPackageInfo(const std::string& packageId) {
#if BEATCONNECT_USE_JUCE
        juce::URL url(juce::String(config.apiBaseUrl) +
                      "/packages/" + juce::String(packageId));
        ...
        if (obj->hasProperty("name"))
            info.name = obj->getProperty("name").toString().toStdString();
        if (obj->hasProperty("package_type"))
            info.type = obj->getProperty("package_type").toString().toStdString();
            ...
            if (auto* arr = items.getArray()) {
                for (const auto& item : *arr) {
                    if (auto* itemObj = item.getDynamicObject()) {
                        AssetInfo asset;
                        if (itemObj->hasProperty("content_id"))
                            asset.id = itemObj->getProperty("content_id")
                                .toString().toStdString();
                        if (itemObj->hasProperty("name"))
                            asset.name = itemObj->getProperty("name")
                                .toString().toStdString();
                        if (itemObj->hasProperty("file_size")) {
                            asset.fileSize = (int64_t)itemObj->getProperty("file_size");
                            info.totalSize += asset.fileSize;
                        }
    ...
    void downloadPackage(
        ...
        AssetDownloader::ProgressCallback progressCallback,
        AssetDownloader::BatchCompletionCallback completionCallback
    ) {
        auto info = getPackageInfo(packageId);
        if (!info || info->assets.empty()) {
            if (completionCallback) {
        ...
        assetDownloader.downloadBatch(assetIds, progressCallback, completionCallback);
    }

    bool isPackageDownloaded(const std::string& packageId) const {
        auto info = const_cast<Impl*>(this)->getPackageInfo(packageId);
        if (!info) return false;

        for (const auto& asset : info->assets) {
            if (!assetDownloader.isDownloaded(asset.id)) {
                return false;
            }
        }
        return true;
...
PackageDownloader::PackageDownloader() : pImpl(std::make_unique<Impl>()) {}
PackageDownloader::~PackageDownloader() = default;

void PackageDownloader::configure(const DownloaderConfig& config) {
    pImpl->configure(config);
}

std::optional<PackageDownloader::PackageInfo>
PackageDownloader::getPackageInfo(const std::string& packageId) {
    return pImpl->getPackageInfo(packageId);
}

void PackageDownloader::downloadPackage(
    const std::string& packageId,
    AssetDownloader::ProgressCallback progressCallback,
    AssetDownloader::BatchCompletionCallback completionCallback
) {
    pImpl->downloadPackage(packageId, progressCallback, completionCallback);
}

bool PackageDownloader::isPackageDownloaded(const std::string& packageId) const {
    return pImpl->isPackageDownloaded(packageId);
}
